...
function getYARRRMLTemplateFromTermMap(tm) {
  ...
  const templates = store.getQuads(tm, namedNode(namespaces.rr + 'template'), null).map(a => a.object);

  if (constants.length > 0) {
    return collapseNamespace(constants[0].value);
  } else if (references.length > 0) {
    return `$(${references[0].value})`;
  } else if (templates.length > 0){
    return convertR2RMLTemplateToYARRRMLTemplate(templates[0].value);
  }
