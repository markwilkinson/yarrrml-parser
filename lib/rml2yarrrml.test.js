...
const toYARRRML = require('./rml2yarrrml.js');
...
//   });
  ...
  it('Works for rml2yarrrml_1', (done) => {
    doToYARRML(path.resolve(__dirname, '../test/rml2yarrrml_1/mapping_reverse.ttl'), path.resolve(__dirname, '../test/rml2yarrrml_1/mappings.yarrrml'), done);
  });

  it('Works for rml2yarrrml_2', (done) => {
    doToYARRML(path.resolve(__dirname, '../test/rml2yarrrml_2/mapping_reverse.ttl'), path.resolve(__dirname, '../test/rml2yarrrml_2/mappings.yarrrml'), done);
  });

  function doToYARRML(rmlPath, yarrrmlPath, done) {
    const rml = fs.readFileSync(rmlPath, 'utf8');
    ...
    parser.parse(rml, (err, quad, prefixes) => {
      if (quad) {
        ...
        toYARRRML(quads, prefixes).then(str => {
          const output = fs.readFileSync(yarrrmlPath, 'utf8');
          try {
